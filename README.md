...
- 在菜单栏显示额度和恢复倒计时，并用相同高度、颜色和间距的分割线区隔百分比、时间与活动圆环
- 在菜单栏用旋转圆环显示本机是否仍有未完成的 Codex 任务
- 在弹出面板里查看周额度
- Codex 写入本机日志后，经 800 毫秒事件防抖刷新；60 秒轮询作为兜底
- 在弹出面板中查看精确到个位的今日 Token 总量，以及高精度的缓存输入、非缓存输入、输出和推理明细
...
今日 Token 用量只读取 `payload.type == "token_count"` 记录中的 `payload.info.last_token_usage` 字段，用来汇总今日总量、缓存输入、非缓存输入、输出和推理明细。面板总量使用带千位分隔符的精确整数，分项以三位小数的“亿”或两位小数的“万”显示，避免后台已刷新但大数值因四舍五入看起来没有变化。
